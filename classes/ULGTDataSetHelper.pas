...
begin
  ...
  begin
    ...
    for lCol := 0 to Pred(FieldCount) do
    begin
      ...
      begin
        ...
        begin
          if Fields[lCol].DataType in [ftCurrency, ftFloat, ftInteger, ftSmallint, ftSingle ]
          then lJO.AddPair(Fields[lCol].FieldName, TJSONNumber.Create(Fields[lcol].Value))
          else lJO.AddPair(fields[lcol].FieldName, fields[lcol].Value);
        end;
      end;
    end;
...
procedure TDataSetHelper.SaveToJSON(const AFileName: string);
var
  S : TStringList;
begin
